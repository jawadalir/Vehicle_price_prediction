...
model_files_exist = all([
    ...
    os.path.exists("models/weights_nnls.joblib"),
    os.path.exists("models/models_per_fold.joblib"),
...
if not model_files_exist:
    ...
    st.write("- models/features.joblib")
    st.write("- models/scaler.joblib")
    st.write("- models/weights_nnls.joblib")
    st.write("- models/models_per_fold.joblib")
    st.write("- output.json (in root folder)")
    st.stop()
...
def load_models():
    ...
    try:
        ...
        weights = joblib.load("models/weights_nnls.joblib")
        models_per_fold = joblib.load("models/models_per_fold.joblib")
        
        # Load model rankings from JSON file
        with open("output.json", "r") as f:
            model_rankings = json.load(f)
        
        return features, scaler, weights, models_per_fold, model_rankings, True
...
with st.spinner("Loading ML models and data..."):
    features, scaler, weights, models_per_fold, model_rankings, loaded = load_models()
...
if st.button("🚀 PREDICT CAR PRICE", type="primary", use_container_width=True):
    ...
    with st.spinner("Processing input data..."):
        ...
        # --------------------------------------------------------
        # 1️⃣1️⃣ Feature Engineering (MUST match training)
        # --------------------------------------------------------

        df = pd.DataFrame([test_car])

        # 1. Basic mileage/age features (MUST match training)
        ...
        # 2. Brand/model features (MUST match training)
        ...
        # 3. Emission features (MUST match training)
        ...
        # --------------------------------------------------------
        # 1️⃣2️⃣ Fill Missing Features and Scale
        # --------------------------------------------------------
        ...
        with st.spinner("Making predictions with ensemble models..."):
            # Make prediction using all folds
            all_predictions = []
            for fold_models in models_per_fold:
                fold_preds = []
                for model_name in ['xgb', 'lgb', 'cat', 'extra']:
                    model = fold_models[model_name]
                    pred_log = model.predict(X_scaled)[0]  # Log space prediction
                    fold_preds.append(pred_log)
                all_predictions.append(fold_preds)

            # Average predictions across folds
            all_predictions = np.array(all_predictions)  # shape: (n_folds, 4_models)
            avg_log_predictions = np.mean(all_predictions, axis=0)

            # Convert from log to euros
            avg_eur_predictions = np.expm1(avg_log_predictions)

            # Apply ensemble weights
            predicted_price = np.dot(avg_eur_predictions, weights)
        ...
        for i, (col, name, price) in enumerate(zip(model_cols, model_names, avg_eur_predictions)):
            with col:
                col.metric(name, f"€{price:,.2f}", f"Weight: {weights[i]:.2%}")

        st.info(f"**Ensemble weights:** {weights}")
...
st.markdown(
    ...
    <div style='text-align: center; color: gray;'>
        <p>🚗 Car Price Prediction System • Using Ensemble ML Model</p>
        <p><small>XGBoost • LightGBM • CatBoost • ExtraTrees with weighted ensemble</small></p>
    </div>
    """,
